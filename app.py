...
V1's own modules are untouched; they are imported lazily below so that the app
still starts in an offline environment where google-genai is not installed --
which is the normal state of the V2 virtualenv.
...
# V1 depends on google-genai, which the offline V2 environment deliberately
# does not install. Importing it eagerly would take the whole app down --
# including V2 -- so failure here only disables the V1 routes.
try:
...
def home(request: Request):
    if not V1_AVAILABLE:
        # Nothing to fall back to on this install; send users to the pipeline
        # that does work rather than rendering a form that cannot submit.
...
def v2_health():
    ...
    return {
        ...
        "v1_import_error": V1_IMPORT_ERROR or None,
