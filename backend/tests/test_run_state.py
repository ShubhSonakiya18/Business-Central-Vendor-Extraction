...
def run_id():
    ...
    rid = "abcdef0123"
    yield rid
    shutil.rmtree(OUTPUT_DIR / rid, ignore_errors=True)
...
class TestRoundTrip:
    def test_saved_state_reads_back_identical(self, run_id):
        ...
        web.save(run_id, state)
        assert web.load(run_id) == state

    def test_state_lands_under_the_run_directory(self, run_id):
        web.save(run_id, {"files": {}})
...
class TestRunIdGuard:
    ...
    def test_rejects_anything_but_a_generated_id(self, bad_id):
        assert web.load(bad_id) is None
    ...
    def test_traversal_cannot_read_a_file_outside_outputs(self, tmp_path):
        ...
        """
        outside = OUTPUT_DIR.parent / "pytest_outside_run"
        ...
        try:
            ...
            assert web.load(traversal_id) is None
        finally:
    ...
    def test_case_variant_cannot_reach_a_real_run(self, run_id):
        ...
        web.save(run_id, {"files": {"json": "x"}})
        assert web.load(run_id.upper()) is None

    def test_accepts_the_format_the_app_generates(self, run_id):
        web.save(run_id, {"files": {}})
        assert web.load(run_id) is not None
