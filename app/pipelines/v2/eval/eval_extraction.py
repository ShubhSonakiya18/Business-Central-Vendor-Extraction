...
def extract_v2(document_dir: Path, models: str = "small", cache: Optional[Path] = None) -> dict:
    ...
    from ..pipeline import collect_inputs, extract_from_document_set
...
def extract_v1(document_dir: Path) -> dict:
    """V1 (Gemini) needs the three documents identified by role, and sends them
    to Google -- see the data-sensitivity notes in the README before running
    this against real vendor documents."""
    # build_vendor_json reads GEMINI_API_KEY straight from os.environ, and
    # nothing on this code path imports app.core.config, which is what normally
    # loads .env. Load it here so the harness works the same way the app does.
    from app.core.config import settings  # noqa: F401  (import side effect: load_dotenv)
    from app.services.vendor_v1_gemini import build_vendor_json

    if not os.environ.get("GEMINI_API_KEY"):
        raise SystemExit(
            "GEMINI_API_KEY is not set (checked .env and the environment). "
            "V1 cannot run without it."
        )

    def find(*keywords: str) -> Path:
        for p in sorted(document_dir.iterdir()):
            name = p.name.lower()
            if p.suffix.lower() == ".pdf" and any(k in name for k in keywords):
                return p
        raise SystemExit(f"Could not find a document matching {keywords} in {document_dir}")

    return build_vendor_json(
        str(find("cheque")), str(find("gst")), str(find("udyam"))
    )
...
def main() -> int:
    ...
    parser.add_argument("--cache", help="Cache/reuse the OCR'd document set at this path (V2 only)")
    ...
    print(f"Running {args.pipeline.upper()} extraction over {document_dir}...")
    if args.pipeline == "v2":
        extracted = extract_v2(document_dir, args.models, Path(args.cache) if args.cache else None)
    else:
        extracted = extract_v1(document_dir)
    ...
    if args.json_out:
        ...
        out.write_text(
            json.dumps(
                {"vendor_id": ground_truth.get("vendor_id"), "pipeline": args.pipeline,
