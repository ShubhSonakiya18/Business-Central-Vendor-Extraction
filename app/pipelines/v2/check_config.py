...
2. Field keys still line up with the canonical vendor JSON that V1's Excel
   writer expects, so the mapping layer stays compatible (requirement 9).
3. Configured patterns actually match the real values OCR'd out of the sample
...
def v1_canonical_keys() -> list[str]:
    """Read XLSX_CELL_MAP out of the V1 module without importing it.

    Importing would pull in google-genai, which is deliberately absent from
    this offline environment -- so parse the source instead.
    """
    tree = ast.parse(V1_SOURCE.read_text(encoding="utf-8"))
    for node in ast.walk(tree):
        if isinstance(node, ast.Assign):
            for target in node.targets:
                if isinstance(target, ast.Name) and target.id == "XLSX_CELL_MAP":
                    return list(ast.literal_eval(node.value))
    raise RuntimeError(f"XLSX_CELL_MAP not found in {V1_SOURCE.name}")
...
def main() -> int:
    ...
    print()
    print("=" * 74)
    print("3. CANONICAL KEY COMPATIBILITY (vs V1 XLSX_CELL_MAP)")
    print("=" * 74)
    v1_keys = v1_canonical_keys()
    cfg_keys = set(dictionary.keys)
    missing = [k for k in v1_keys if k not in cfg_keys]
    extra = sorted(cfg_keys - set(v1_keys))

    print(f"  V1 mapped cells      : {len(v1_keys)}")
    print(f"  V2 dictionary fields : {len(cfg_keys)}")
    if missing:
        failures.append(f"fields in V1's Excel map but missing from the dictionary: {missing}")
        print(f"  FAIL  missing: {missing}")
    else:
        print("  OK    every V1 Excel field is present in the dictionary")
    if extra:
        print(f"  note  V2-only fields (not in V1 Excel map): {extra}")

    # -- 4. patterns vs real values ----------------------------------------
    print()
    print("=" * 74)
    print("4. PATTERNS vs REAL EXTRACTED VALUES")
    print("=" * 74)
